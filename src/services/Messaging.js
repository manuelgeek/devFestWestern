...
export function messaging (firebase){
    ...
    if(firebase.messaging.isSupported()){
        ...
        if (Notification.permission === "granted") {
            ...
            //? add window overlay
            const el = document.getElementsByClassName('sidenav-overlay1')[0];
            // console.log(el)
        ...
        function isTokenSentToServer() {
            return window.localStorage.getItem('sentToServer_DevWest') === '1';
        }

        // eslint-disable-next-line no-inner-declarations
        function setTokenSentToServer(sent) {
            window.localStorage.setItem('sentToServer_DevWest', sent ? '1' : '0');
        }
